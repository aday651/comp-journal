...
(Below in the section about the relationship between the change in
rent and the change in those in an income bracket is a description
of the procedure we use to do this analysis.) 
...
We see that there is a weak (but statistically significant)
negative correlation between the change in number of households earning
less than $50,000, and the rate at which rent has increased (after
...
df_rent_lower_quartile_tracts <- map_dfr(
    ...
    select(GEOID, year, estimate, moe) %>%
    pivot_wider(
        names_from = year, 
        values_from = c(estimate, moe),
        names_prefix = "rent_"
    ...
    mutate(
        estimate_rent_2010_in_2019_dollars = estimate_rent_2010 * 1.175,
        ratio_adjusting_for_inflation = estimate_rent_2019 / (estimate_rent_2010 * 1.175),
        rent_log_ratio_moe = sqrt( (moe_rent_2019 / estimate_rent_2019)**2 + (moe_rent_2010 / estimate_rent_2010)**2 )
    )

df_ratio_change_tracts <- df_rdincome_total_change_tracts %>%
    select(-geometry) %>%
    filter(variable %in% c("Less than $50,000")) %>%
    select(GEOID, ratio, log_ratio_moe)

df_tract_changes <- left_join(
    df_rent_lower_quartile_tracts, df_ratio_change_tracts
)

df_tract_changes %>% 
    ggplot(aes(
        x = ratio_adjusting_for_inflation, y = ratio)
    ) +
    scale_x_log10() + scale_y_log10() +
    geom_point()

ct <- cor.test(
    log(df_tract_changes$ratio_adjusting_for_inflation),
    log(df_tract_changes$ratio),
    use = "complete.obs"
)
...
dr <- deming(
    log(ratio_adjusting_for_inflation) ~ log(ratio), data = df_tract_changes,
    xstd = log_ratio_moe, ystd = rent_log_ratio_moe
)
dr

dr2 <- deming(
    log(ratio) ~ log(ratio_adjusting_for_inflation), data = df_tract_changes,
    ystd = log_ratio_moe, xstd = rent_log_ratio_moe
)
dr2
```
...
across the tracts, we also see that there is a statistically 
significant relationship between the two ratios.
...
dr2 <- deming(
    log(ratio.x) ~ log(ratio.y), data = df_rent_cor_test,
    ystd = log_ratio_moe.x, xstd = log_ratio_moe.y
)
dr2
```
